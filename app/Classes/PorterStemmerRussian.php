...
define('PREFIX', '/^(без|бес|возо|воз|вос|вз|во|вс|вы|в|до|за|изо|из|ис|недо|надо|низо|над|наи|низ|нис|на|не|обез|обес|обо|об|о|ото|от|па|пра|по|под|подо|пере|пре|пред|предо|при|про|раз|рас|разо|со|су|с|через|черес|чреза|ана|анти|архи|гипер|гипо|де|дез|дис|ин|интер|инфра|квази|кило|контр|макро|микро|мега|мата|мульти|орто|пан|пара|пост|прото|ре|суб|супер|транс|ультра|зкстра|экс)/'.MODE);
...
class PorterStemmerRussian
{
    ...
    public static function word(&$word)
    {
        ...
        $word = $start.self::$RV;
        $word = preg_replace(PREFIX, '', mb_strtolower($word));
        self::$RV = '';
